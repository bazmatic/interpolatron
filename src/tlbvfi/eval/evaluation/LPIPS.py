...
# Try MPS first, then CUDA, then CPU
if torch.backends.mps.is_available():
    device = torch.device('mps')
    print("📊 LPIPS using Apple Silicon GPU (MPS)")
elif torch.cuda.is_available():
    device = torch.device('cuda:0')
    print("📊 LPIPS using CUDA GPU")
else:
    device = torch.device('cpu')
    print("📊 LPIPS using CPU")
...
def calc_LPIPS(data_dir, gt_dir, num_samples=1):
    ...
    for i in tqdm(range(total), total=total, smoothing=0.01):
        ...
        gt_img = lpips.im2tensor(lpips.load_image(gt_name)).to(device)
        for j in range(num_samples):

            img_name = os.path.join(os.path.join(data_dir,f'{j}', f'sample_from_next{str(i)}.png'))

            img_calc = lpips.im2tensor(lpips.load_image(img_name)).to(device)
